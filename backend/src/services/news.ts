...
function isRelevantIndianLoanNews(title: string, description: string): boolean {
...
function importanceScore(title: string, description: string, pubDate?: string): number {
  const text = `${title} ${description}`.toLowerCase();
  let score = 0;
  if (text.includes('rbi')) score += 8;
  if (text.includes('interest rate')) score += 6;
  if (text.includes('home loan') || text.includes('housing loan')) score += 6;
  if (text.includes('sbi') || text.includes('hdfc') || text.includes('icici')) score += 4;
  if (text.includes('real estate') || text.includes('property')) score += 3;
  try {
    if (pubDate) {
      const diffHrs = (Date.now() - new Date(pubDate).getTime()) / 3_600_000;
      if (diffHrs < 1) score += 6; else if (diffHrs < 6) score += 4; else if (diffHrs < 24) score += 2;
    }
  } catch {}
  return score;
}
...
export async function getLoanNews(): Promise<NewsItem[]> {
...
export async function streamLoanNewsSSE(_req: any, res: any): Promise<void> {
  res.writeHead(200, {
